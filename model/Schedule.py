...
class Schedule:
    ...
    def makeNewFromPrototype(self, positions = None):
        ...
        for c in classes:
            ...
            # hard code to not get flag ceremony and lunch time
            while True:
                ...
                if c.room_type == 'Y1':
                    day = 0
                    room = nr-24 + next(iter(c.Groups)).Id - 1
                else:
                    room = randrange(nr-24)

                if not (day==2 and time==0) and not ((time in (10,11)) or ((time + dur - 1) in (10,11)) or ((time < 10) and (time + dur -1 > 10))):
                    break
    ...
    def repair(self, cc1: CourseClass, reservation1_index: int, reservation2: Reservation):
        ...
        if reservation2 is None:
            while True:
                ...
                if cc1.room_type == 'Y1':
                    day = 0
                    room = nr-24 + next(iter(cc1.Groups)).Id - 1
                else:
                    room = randrange(nr-24)
    ...
    def calculateFitness(self):
        ...
        for cc, reservation_index in items:
            ...
            ro = Criteria.isRoomOverlapped(cc, slots, reservation, dur)
            ...
            #[ci + 1] = Criteria.isSeatEnough(r, cc)

            # does current room have computers if they are required
            criteria[ci + 1] = Criteria.isComputerEnough(r, cc)
            ...
            criteria[ci + 3] = not go

            # check if class is during lunch time
            criteria[ci + 4] = not Criteria.isOverlappedLunch(time, dur)

            # Online Classes on Monday
            criteria[ci + 5] = Criteria.isOnlineClassMonday(day, cc)

            # Class is on Flag Ceremony
            criteria[ci + 6] = not Criteria.isOverlappedFlagCeremony(day, time)
